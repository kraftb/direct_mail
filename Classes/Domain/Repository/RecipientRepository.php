...
/**
 * This class should get extended by "RecipientRepository" instead of RecipientRepositoryCompatibility
 */
class RecipientRepositoryRefactored {
	...
	public function findByRecipientGroup(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup, $dmailer) {
		switch ((int)$recipientGroup->getType()) {
	 ...
	 */
	public function findByRecipientFromPage(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup) {
		$pages = $this->getRecipientPages($recipientGroup);
		$result = $this->objectManager->get('TYPO3\CMS\Extbase\Persistence\ObjectStorage');
		if (count($pages)) {
			$whichTables = $recipientGroup->getRecordTables();

			if ($whichTables->contains(new RecipientTableType(RecipientTableType::TT_ADDRESS))) {
			...
			if ($whichTables->contains(new RecipientTableType(RecipientTableType::FE_USERS))) {
				$recipients = $this->frontendUserRepository->findByPagesAndCategories($pages, $recipientGroup->getSelectedCategories());
				$result->addAll($recipients);
			}
			if ($whichTables->contains(new RecipientTableType(RecipientTableType::CUSTOM_TABLE))) {
				$recipients = $this->customRecipientRepository->findByPagesAndCategories($pages, $recipientGroup->getSelectedCategories());
				$result->addAll($recipients);
			}
			if ($whichTables->contains(new RecipientTableType(RecipientTableType::FE_GROUPS))) {
	...
	protected function getRecipientPages(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup) {
		...
		foreach ($pages AS $pageUid) {
			...
			if ($this->pageRepository->readPageAccess($page)) {
				if ($recipientGroup->getPagesRecursive(-1)) {
	 ...
	 */
	public function findByRecipientPlainList(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup) {
		if ($recipientGroup->getPlainRecipientListFormat()->equals(RecipientList::CSV))	{
	 ...
	 */
	public function findByRecipientStaticList(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup) {
		$result = $this->objectManager->get('TYPO3\CMS\Extbase\Persistence\ObjectStorage');
		...
		$result->addAll($this->addressRepository->findByUidList($uidList);

		$uidList = DirectMailUtility::getStaticIdList('fe_users', $recipientGroup->getUid());
		$result->addAll($this->frontendUserRepository->findByUidList($uidList);
		...
		if ($this->userTable) {
			...
			$result->addAll($this->mapCustomRecordsFromDatabase($this->userTable, $uidList);
		}
	 ...
	 * @return \TYPO3\CMS\Extbase\Persistence\QueryResultInterface<\DirectMailTeam\DirectMail\Domain\Model\Interfaces\Recipient> The found recipients
	 */
	public function findByRecipientSpecialQuery(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup) {
	 ...
	 */
	public function findByRecipientOtherGroups(\DirectMailTeam\DirectMail\Domain\Model\RecipientGroup $recipientGroup) {
