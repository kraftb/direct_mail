...
class SendQueueState extends \TYPO3\CMS\Core\Type\Enumeration {
	 ...
	 */
	public function nextState(\DirectMailTeam\DirectMail\Domain\Type\SendQueueState $state) {
		$currentClass = get_called_class();
		if ($state instanceof \DirectMailTeam\DirectMail\Domain\Type\SendQueueState) {
	 ...
	 */
	public function previousState(\DirectMailTeam\DirectMail\Domain\Type\SendQueueState $state) {
		$currentClass = get_called_class();
		if ($state instanceof \DirectMailTeam\DirectMail\Domain\Type\SendQueueState) {
			$stateValue = (int)((string)$state);
		} else {
