...
class RecipientGroup extends \TYPO3\CMS\Extbase\DomainObject\AbstractEntity {
	...
	/**
	 * The recipient table(s) for this recipient group. Will get stored as integer bitmask.
	 *
	 * @var integer
	 */
	...
	/**
	 * Format of plain recipient list
	 *
	...
	/**
	 * Returns a parsed recipient tables ObjectStorage containing a "RecipientTableType" object for
	 * every assigned recipient table.
	 *
	 ...
	 */
	public function getParsedRecipientTables() {
		if ($this->_parsedRecipientTables !== NULL) {
			return $this->_parsedRecipientTables;
		}
		$objectManager = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\CMS\Extbase\Object\ObjectManager');
		$class = 'DirectMailTeam\DirectMail\Domain\Type\RecipientTableType';
		$result = $objectManager->get('TYPO3\CMS\Extbase\Persistence\ObjectStorage');
		if ($this->recipientTables & RecipientTableType::TT_ADDRESS) {
			$result->attach($objectManager->get($class, RecipientTableType::TT_ADDRESS));
		}
		if ($this->recipientTables & RecipientTableType::FE_USERS) {
			$result->attach($objectManager->get($class, RecipientTableType::FE_USERS));
		}
		if ($this->recipientTables & RecipientTableType::CUSTOM_TABLE) {
			$result->attach($objectManager->get($class, RecipientTableType::CUSTOM_TABLE));
		}
		if ($this->recipientTables & RecipientTableType::FE_GROUPS) {
			$result->attach($objectManager->get($class, RecipientTableType::FE_GROUPS));
		}
		$this->_parsedRecipientTables = $result;
		return $this->_parsedRecipientTables;
	}
	 ...
	 *
	 * @return integer The recipient tables assigned to this group
	 */
	...
	/**
	 * Sets recipient tables assigned to to this group
	 *
	 * @param integer $recipientTables: The recipient tables from which to retrieve records
	 * @return void
	 */
	public function setRecipientTables($recipientTables) {
	 ...
	 *
	 * @return \TYPO3\CMS\Extbase\Persistence\ObjectStorage<\DirectMailTeam\DirectMail\Domain\Model\Category> The categories the recipient must have selected
	 */
	 ...
	 *
	 * @param \TYPO3\CMS\Extbase\Persistence\ObjectStorage<\DirectMailTeam\DirectMail\Domain\Model\Category> $selectedCategories: The categories the recipient must have selected
	 * @return void
	 */
