...
class MassMailingService extends \DirectMailTeam\DirectMail\Service\ServiceBase {
	 ...
	 * @return void
	 */
	protected function handleJobs() {
		$start = GeneralUtility::milliseconds();

		// Log invocation of this dmailer run.
		$logMessage = $this->translate('dmailer_invoked_at') . date('h:i:s d-m-Y'); 
	...
	protected function handleJob(\DirectMailTeam\DirectMail\Domain\Model\MailJob $mailJob) {
		...
		if (!$mailJob->getScheduledBegin()) {
			...
			throw new Exception('Everything fine. Job started. "getScheduledBegin" can return FALSE.');
	...
	protected function sendToRecipient(\DirectMailTeam\DirectMail\Domain\Model\SendQueue $queuedItem, \DirectMailTeam\DirectMail\Domain\Model\MailJob $mailJob) {
		...
		if (!$this->advanceQueueItemState($queueItem, $processingState)) {
		...
		$recipientRow['sys_dmail_categories_list'] = $this->dmailer->getListOfRecipentCategories($table, $recipientRow->getUid());
		$this->dmailer->shipOfMail($mailJob->getUid(), $recipientRow, $tKey);
		// ---------- Legacy code -------- end -------------------

		if (!$this->advanceQueueItemState($queueItem, $sentState)) {
			// Could not advance queue item to next state.
...
exit();
	...
	protected function advanceQueueItemState(\DirectMailTeam\DirectMail\Domain\Model\SendQueue $queuedItem, \DirectMailTeam\DirectMail\Domain\Type\SendQueueState $targetState) {
		...
		if ($currentQueuedItem->getState()->equals($previousState)) {
			$currentQueuedItem->setState($targetState);
	...
	protected function updateJobLogAndNotify(\DirectMailTeam\DirectMail\Domain\Model\MailJob $mailJob, $status) {
		...
		switch($status) {
			case 'begin':
				$subject = $this->translate('dmailer_mid') . ' ' . $mailerJob->getUid() . ' ' . $this->translate('dmailer_job_begin');
				$message = $this->translate('dmailer_job_begin') . ': ' . strftime('%d-%m-%Y, %H:%M:%S');
			break;

			case 'end':
				$subject = $this->translate('dmailer_mid') . ' ' . $mailerJob->getUid() . ' ' . $this->translate('dmailer_job_end');
		...
		if ($this->notificationJob) {
			$this->sendNotificationMail($subject, $message);
		}
	}
	 ...
	 * @return void
	 */
	protected function sendNotificationMail($subject, $message) {
	// @todo: Where do "from_name", etc. come from?
